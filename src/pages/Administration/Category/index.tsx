...
class CategoryPage extends React.Component<any,State> {
    columns : Array<any>;
    status : string;
    upID :string;
    upName :string;
    ...
    handleSearch (value :string ,page :number = 1) {
      ...
      Category.search(value , page).then(({success ,data ,pagination}) => {
        this.setState({loading :false})
        if(success && data && pagination) {
            this.setState({data : data ,pagination : { total :pagination.total , current :pagination.current}})
        }
    ...
    save =(record :any) => {
      ...
      if(this.status === 'create'){     
        Category.save(this.state.label,this.props.AdminReducer.token).then(({ success ,data ,msg }) => {
        ...
        if(this.upID && this.upName){
          Category.update(this.upID,this.state.label,this.upName,this.props.AdminReducer.token).then(({success , data ,msg}) => {
            this.setState({opLoading :false})
            if(success && data){
    ...
    remove = (record :any ,index :number) => {
      ...
      Category.remove(record._id,this.props.AdminReducer.token).then(({success ,data ,msg}) => {
        this.setState({loading :false})
        if(success && data){
