...
class BookPage extends React.Component<any,any> {
      ...
      if(record.name){
        ...
        if (target) {
          ...
          if(target.key === 'newKey'  && target.id === 'newKey'){
            Book.save(record.name).then(({success ,data}) => {
                if(success && data){
            ...
            if(record.oldName !== record.name){
              Book.update(record.id).then(({success ,data}) => {
                if(success && data){
                      target.id = target.key = data.id;
                      target.name = data.name
                      this.setState({ data: newData });
    ...
    handleSearch (value :string ) {
      Book.search(value).then(({success ,data ,pagination}) => {
        if(success && data && pagination) {
            let newData = data.map((item :any) => {
                item.key = item.key || item.id;
                item.oldName = item.name;
